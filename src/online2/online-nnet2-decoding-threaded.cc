...
SingleUtteranceNnet2DecoderThreaded::SingleUtteranceNnet2DecoderThreaded(
  ...
  feature_pipeline_.SetTransform(gmm_adaptation_state_.transform);
  // spawn threads.
...
bool SingleUtteranceNnet2DecoderThreaded::RunDecoderSearchInternal() {
  ...
  while (true) {  // decode at most one frame each loop.
    ...
    if (decodable_.NumFramesReady() <= num_frames_decoded) {
      ...
      {  // possibly estimate fMLLR. TODO This code is in another thread than feuture_pipeline!! Use mutex! to set fmllr
        int32 new_frames = num_frames_decoded;
        ...
        bool is_first_utterance_of_speaker = (gmm_orig_adaptation_state_.transform.NumRows() == 0);
...
void SingleUtteranceNnet2DecoderThreaded::EstimateFmllr(bool end_of_utterance) {
  if (decoder_.NumFramesDecoded() == 0) {
...
bool SingleUtteranceNnet2DecoderThreaded::GetGaussianPosteriors(bool end_of_utterance,
                                                      GaussPost *gpost) {
...
bool SingleUtteranceNnet2DecoderThreaded::HaveTransform() {
  return (feature_pipeline_.HaveFmllrTransform());
}

void SingleUtteranceNnet2DecoderThreaded::GetGmmAdaptationState(
